...
from preprocessing_dataset import load_data
...
menu=st.sidebar.radio("Select option",["Main","Parameters","StandardScaler","SimpleImputer","Metrics"])

if menu == "Main":
    st.header("Datasets")
    min_year=int(df["S.No"].min())
    max_year=int(df["S.No"].max())

    year_range=st.slider("Filter Range",min_year,max_year,(min_year,max_year))
    filtering_dataset=df[(df["S.No"]>=year_range[0])&(df["S.No"]<=year_range[1])]
...
elif menu == "StandardScaler":
    st.header("Standard Scaler")
    scaler = StandardScaler()
    X= scaler.fit_transform(X)
    st.write(X)

elif menu == "SimpleImputer":
